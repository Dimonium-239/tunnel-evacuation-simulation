...
for x in range(int(MARGIN/2), TUNNEL_LEN+int(MARGIN/2), CELL_SIZE):
    tmpSmokeYCells = []
    for y in range( int((WIN_HEIGHT-NUM_OF_PATHES*PATH_LEN)/2), int(((WIN_HEIGHT-NUM_OF_PATHES*PATH_LEN)/2)+PATH_LEN*NUM_OF_PATHES), CELL_SIZE ):
        if isFire == False and (x > badCar.rect[0] and x < badCar.rect[0]+int(1.5*CELL_SIZE)-1) and (y > badCar.rect[1] and y < badCar.rect[1]+int(1.5*CELL_SIZE)):
...
tunEnter = SafeZones((TUNNEL_LEN+MARGIN/2, (WIN_HEIGHT-NUM_OF_PATHES*PATH_LEN)/2), ( EXIT_SIZE, PATH_LEN*NUM_OF_PATHES) )

j = 1
for i in range(0, TUNNEL_LEN):
    if(i % meters_to_pixels(500) == 0):
        topSZ = SafeZones((MARGIN/2+meters_to_pixels(500)*j, (WIN_HEIGHT-NUM_OF_PATHES*PATH_LEN)/2 - EXIT_SIZE/2), ( EXIT_SIZE*2, EXIT_SIZE) )
        downSZ = SafeZones((MARGIN/2+meters_to_pixels(500)*j, (WIN_HEIGHT-NUM_OF_PATHES*PATH_LEN)/2 - EXIT_SIZE/2 + NUM_OF_PATHES*PATH_LEN), ( EXIT_SIZE*2, EXIT_SIZE) )
